...
y <- scale(df_hrv$RMSSD) # dependent variable y
...
(beta_result[1,2] * sqrt(var(df_hrv$RMSSD, na.rm = T)) / 
        sqrt(var(original_weather$min_temp, na.rm = T)))
...
df_pred <- make_ci_df(mcmc_result$y_pred, y_filled, is_pred = T, T = T, num_pred = num_pred)
